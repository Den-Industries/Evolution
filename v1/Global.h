...
#define BackGroundDots 1500
#define BackGroundMovevall 80
#define UpdateCellsCDvall 30
#define CellStoragevall 24
#define SpawnACompvall 15
#define CellAgeToDie 1200
...
int ScreenSize[2], camfollow = -1, gen = 0;
float BGmovecd = 0, UpdateCellsCD = 0, SpawnACompCD = 0, mousecd = 0;
RenderWindow window;
View view;
Font font; 
Text text("", font, 48);
...
void ViewUpdate(float TimesGone)
{
	...
	view.setCenter(view.getCenter().x + (max(abs(MousePos.x - (ScreenSize[0]) / 2), ScreenSize[0] / 4) - ScreenSize[0] / 4)
		* (MousePos.x - (ScreenSize[0]) / 2) * abs((MousePos.x - (ScreenSize[0]) / 2)) * 0.000000001 * TimesGone,
		view.getCenter().y + (max(abs(MousePos.y - (ScreenSize[1]) / 2), ScreenSize[1] / 4) - ScreenSize[1] / 4)
		* (MousePos.y - (ScreenSize[1]) / 2) * abs((MousePos.y - (ScreenSize[1]) / 2)) * 0.000000004 * TimesGone);
